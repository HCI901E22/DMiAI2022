...
def main():
    ...
    model = agent(env.observation_space.shape,
                  spaces.flatdim(env.action_space))
    # Target Model (updated every 100 steps)
    target_model = agent(env.observation_space.shape,
                         spaces.flatdim(env.action_space))
    ...
    for episode in range(train_episodes):
        ...
        while not done:
            ...
            if random_number <= epsilon:
                ...
                encoded = observation
                encoded_reshaped = encoded.reshape((1, 240))
